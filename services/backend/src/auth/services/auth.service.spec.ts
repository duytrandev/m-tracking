...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    verificationTokenRepository = module.get<Repository<EmailVerificationToken>>(
  ...
  describe('register', () => {
    it('should register a new user successfully', async () => {
      ...
      vi.spyOn(userRepository, 'create').mockReturnValue(mockUser as any);
      vi.spyOn(userRepository, 'save').mockResolvedValue(mockUser);
      vi.spyOn(roleRepository, 'findOne').mockResolvedValue(mockRole);
      vi.spyOn(passwordService, 'generateToken').mockReturnValue('verification-token');
      vi.spyOn(passwordService, 'hashToken').mockReturnValue('hashed-token');
      vi.spyOn(verificationTokenRepository, 'create').mockReturnValue({} as any);
      vi.spyOn(verificationTokenRepository, 'save').mockResolvedValue({} as any);
  ...
  describe('validateUser', () => {
    ...
    it('should throw UnauthorizedException if email not verified', async () => {
      ...
      vi.spyOn(userRepository, 'findOne').mockResolvedValue(unverifiedUser as User);
      vi.spyOn(passwordService, 'compare').mockResolvedValue(true);
  ...
  describe('login', () => {
    it('should generate tokens and create session', async () => {
      ...
      vi.spyOn(tokenService, 'generateRefreshToken').mockReturnValue('refresh-token');
      vi.spyOn(sessionService, 'createSession').mockResolvedValue(mockSession as any);
      vi.spyOn(tokenService, 'generateAccessToken').mockReturnValue('access-token');
      vi.spyOn(sessionService, 'updateRefreshToken').mockResolvedValue(undefined);
  ...
  describe('refresh', () => {
    it('should refresh tokens successfully', async () => {
      ...
      const mockSession = { id: 'session-123', expiresAt: new Date(Date.now() + 1000000) };
      const decodedToken = { sub: 'user-123', sessionId: 'session-123', tokenVersion: 1 };

      vi.spyOn(tokenService, 'verifyRefreshToken').mockResolvedValue(decodedToken);
      vi.spyOn(sessionService, 'findByRefreshToken').mockResolvedValue(mockSession as any);
      vi.spyOn(service, 'findById').mockResolvedValue(mockUser);
      vi.spyOn(tokenService, 'blacklistRefreshToken').mockResolvedValue(undefined);
      vi.spyOn(tokenService, 'generateAccessToken').mockReturnValue('new-access-token');
      vi.spyOn(tokenService, 'generateRefreshToken').mockReturnValue('new-refresh-token');
      vi.spyOn(sessionService, 'updateRefreshToken').mockResolvedValue(undefined);
      ...
      expect(tokenService.blacklistRefreshToken).toHaveBeenCalledWith(oldRefreshToken, 'user-123');
    });

    it('should throw UnauthorizedException if session expired', async () => {
      const expiredSession = { id: 'session-123', expiresAt: new Date(Date.now() - 1000) };
      const decodedToken = { sub: 'user-123', sessionId: 'session-123', tokenVersion: 1 };

      vi.spyOn(tokenService, 'verifyRefreshToken').mockResolvedValue(decodedToken);
      vi.spyOn(sessionService, 'findByRefreshToken').mockResolvedValue(expiredSession as any);
  ...
  describe('logout', () => {
    it('should blacklist tokens and revoke session', async () => {
      ...
      vi.spyOn(tokenService, 'blacklistAccessToken').mockResolvedValue(undefined);
      vi.spyOn(tokenService, 'blacklistRefreshToken').mockResolvedValue(undefined);
      vi.spyOn(sessionService, 'findByRefreshToken').mockResolvedValue(mockSession as any);
      vi.spyOn(sessionService, 'revokeSession').mockResolvedValue(undefined);
      ...
      expect(tokenService.blacklistAccessToken).toHaveBeenCalledWith(accessToken, userId);
      expect(tokenService.blacklistRefreshToken).toHaveBeenCalledWith(refreshToken, userId);
      expect(sessionService.revokeSession).toHaveBeenCalledWith('session-123');
    });
  });
  ...
  describe('verifyEmail', () => {
    it('should verify email with valid token', async () => {
      ...
      vi.spyOn(userRepository, 'update').mockResolvedValue({} as any);
      vi.spyOn(verificationTokenRepository, 'update').mockResolvedValue({} as any);

      const result = await service.verifyEmail(token);

      expect(result.message).toContain('Email verified successfully');
      expect(userRepository.update).toHaveBeenCalledWith('user-123', { emailVerified: true });
    });
  ...
  describe('forgotPassword', () => {
    it('should create reset token and send email', async () => {
      ...
      vi.spyOn(passwordService, 'hashToken').mockReturnValue('hashed-token');
      vi.spyOn(resetTokenRepository, 'create').mockReturnValue({} as any);
      vi.spyOn(resetTokenRepository, 'save').mockResolvedValue({} as any);
  ...
  describe('resetPassword', () => {
    it('should reset password with valid token', async () => {
      ...
      vi.spyOn(passwordService, 'hashToken').mockReturnValue('hashed-token');
      vi.spyOn(resetTokenRepository, 'findOne').mockResolvedValue(mockResetToken as any);
      vi.spyOn(passwordService, 'hash').mockResolvedValue('new-hashed-password');
      vi.spyOn(userRepository, 'update').mockResolvedValue({} as any);
      vi.spyOn(resetTokenRepository, 'update').mockResolvedValue({} as any);
